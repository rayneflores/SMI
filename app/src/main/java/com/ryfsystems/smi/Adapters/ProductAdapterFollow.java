...
public class ProductAdapterFollow extends RecyclerView.Adapter<ProductViewHolder> {

    Activity activity;
    List<Product> productList;

    public ProductAdapterFollow(Activity activity, List<Product> productList) {
    ...
    public ProductViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.product_follow_layout, parent, false);
        ProductViewHolder viewHolder = new ProductViewHolder(itemView);
        return viewHolder;
    }

    @Override
    public void onBindViewHolder(@androidx.annotation.NonNull ProductViewHolder viewHolder, int i) {
        ...
        if (productList.get(i).getPoferta() > 0) {
            pventa.setSpan(new StrikethroughSpan(), 17, (productList.get(i).getPventa().toString().length() + 17), 0);
        }
        ...
        viewHolder.tvListProductCodSuc.setText(codSucursal);
    }
